...
创作者在向导里要填大量结构化字段（立意、**小说概要**、世界观、人物、大纲、章节）。Agent 的目标是：

1. **一句话开书**：把意图拆成对齐现有表单的子任务；**优先落成长文 `story_overview`（全书真源）**，再由概要提取世界观/人物/大纲等。
2. **当前步骤辅助**：只生成本步字段，不越权改其他步骤；有概要时后续步骤对照概要，不另起主线。
3. **单字段润色**：对已有文本做生成 / 扩写 / 改写，用户确认后再写入。
4. **设定体检**（二期）：详情页检查人物/世界观/大纲是否与概要、立意自洽。
