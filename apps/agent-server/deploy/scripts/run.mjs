...
function printHelp() {
  console.log(`Pi Agent (agent-server) — CLI

首次:
  cd apps/agent-server/deploy && npm link

用法:
  ams-agent local              启动 infra + agent-server（profile agent）
  ams-agent local:infra        仅 DB + Redis
  ams-agent local:reset        清库重建
  ams-agent local:down         停止本栈

未 link:
  node apps/agent-server/deploy/scripts/run.mjs <命令>
`)
}
