...
配置生成与部署执行分开：生成走 `ops-project-skill`。部署按项目 `deploy_config.product`：`generic` 仍是 HTTPS 拉 tag + 白名单构建；`agentrun` 写入 AK/环境后跑 `fitness-cli prod` 等价命令；`aliyun_ecs` / `tencent_cvm` 本期 mock。`ops-deploy-skill` 只定合同，不跑构建。
