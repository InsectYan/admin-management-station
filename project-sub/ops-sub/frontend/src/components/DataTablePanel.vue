<template>
  <div
    ref="panelRef"
    class="ops-data-table-panel"
    :style="panelStyle"
  >
    <div
      class="ops-data-table-panel__body"
      :style="bodyStyle"
    >
      <slot
    ...
    <div
      v-if="showFooter"
      ref="footerRef"
      class="ops-data-table-panel__footer"
      :style="footerStyle"
    >
      <el-pagination
        :current-page="page"
...
<style scoped>
.ops-data-table-panel {
  position: relative;
  width: 100%;
}

.ops-data-table-panel__body {
  overflow: hidden;
}

.ops-data-table-panel__footer {
  display: flex;
  justify-content: flex-end;
  align-items: center;
  flex-shrink: 0;
  padding: 12px 0;
  border-top: var(--ops-border-subtle);
  background: var(--ops-color-surface-elevated);
  backdrop-filter: blur(var(--ops-backdrop-blur, 12px));
