<template>
  <el-container :class="['novel-sub-layout', { embedded }]">
    <NavMenu
      v-if="!embedded"
      :embedded="embedded"
      title="小说创作平台"
    />
    <el-container
      :class="['novel-sub-main-column', { embedded }]"
      direction="vertical"
    >
      <div v-if="embedded" class="novel-sub-embedded-bar">
        <NavMenu :embedded="embedded" />
      </div>
      <el-main :class="['novel-sub-content', { embedded }, sceneClass]">
        <router-view />
      </el-main>
    </el-container>
  </el-container>
