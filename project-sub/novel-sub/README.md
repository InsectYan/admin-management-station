...
cd project-sub/novel-sub/backend && cp .env.example .env && npm install && npm run dev
cd project-sub/novel-sub/frontend && npm install && npm run dev
```
...
| PUT  | `/api/novels/:id/setting` | 保存世界观/门派/人物/大纲/章节目录 |
| GET  | `/api/novels/:id`         | 加载草稿（含 setting_json，不含正文） |
| GET  | `/api/novels/:id/chapters` | 章节正文元数据（字数，无正文） |
| GET  | `/api/novels/:id/chapters/:chapterId` | 单章正文 + 上一章/下一章 |
