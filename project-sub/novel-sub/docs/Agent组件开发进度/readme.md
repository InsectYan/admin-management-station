...
> **状态**：A1–A9、B–E 向导、F 开书计划、G 列表/详情、H 多模态封面、I 门派与单章、J 正文进主链路、K 续写/预览/导出、L 连续执行设定已落地  
...
| I1 | 门派组织步 3 + 章节目录改名说明 | 完成 |
| I2 | 正文独立表 + 单章 GET/PUT | 完成 |
| I3 | 详情单章开发（树 / 会话操作岛 / 正文坞 / 上下章） | 完成 |
| J1 | 向导/列表「开始写正文 / 去写正文」进 tab=7 | 完成 |
| J2 | 单章 snapshot：上一章结尾、下一章钩子、目标字数 | 完成 |
| J3 | 「写下一空章」生成 → 应用保存 → 下一空章 | 完成 |
| J4 | 计划 `plan.bodies` + dispatch 跳详情空章；coverage 含门派 | 完成 |
| J5 | 设计文档 08/09/11 与进度 readme | 完成 |
| K1 | 续写未完成章节队列（可暂停） | 完成 |
| K2 | 列表/详情进度按正文派生 | 完成 |
| K3 | 全书预览 tab=8 | 完成 |
| K4 | 导出 TXT / Markdown | 完成 |
| L1 | 开书 / 补全缺口「连续执行设定」到 t_content，停在正文前 | 完成 |
