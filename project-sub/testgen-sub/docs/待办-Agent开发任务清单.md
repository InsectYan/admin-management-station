...
> 对应 [设计-Agent协作与Skill嵌入.md](./设计-Agent协作与Skill嵌入.md) v0.2  

> 状态：`✅` 已实现骨架 / `🔧` 部分 / `❌` 待做  

> **文档索引**：[README.md](./README.md) · **联调配置**：[设计-Agent联调配置.md](./设计-Agent联调配置.md)
...
| 0.8 | 联调探活脚本 | `backend/scripts/agent-linkage-smoke.js` | ✅ |
...
| 1.1 | Skill 插件 judge / explain / pre_review / **summary** | `agent-management-sub/plugins/fitness-judge-skill/` | ✅ |
...
| 1.6 | E6 用例联调（rubric + TS-04） | `backend/scripts/e6-smoke.js` 离线 ✅ · 在线待部署 | 🔧 |
...
| 3.1 | Skill 插件 from_example / expand_matrix / gen_adversarial / **enrich_csv** / **enrich_samples** | `plugins/fitness-sample-skill/` | ✅ |
...
| 3.5 | CSV/JSON 导入 + 智能补全 | 前端 + BFF `enrich-csv` | ✅ |
...
| 4.4 | 人工打分 UI | `ConfigManPanel.vue` | ✅ |

| 4.5 | AI 预审 `pre_review` 按钮 | `ConfigManPanel.vue` | ✅ |
...
| 7.1 | TS-09-LOAD + k6 脚本导出 | `k6ScriptBuilder.js` + `ConfigLoadPanel` | ✅ |

| 7.2 | TS-09 内置压测引擎 | `ts09LoadEngine.js` | ✅ |

| 7.3 | perf `analyze_load_run` | perf-bottleneck-skill | ❌ |
...
- [x] `use_agent_judge: true` → `assertion_detail.agent_judge`（BFF hook 已实现）

- [x] `generate_for_fitness` + 可选样本/dry-run

- [x] 全链路 trace run_id / job_id / item_id

- [x] Agent 宕机 → fail 或 pending，不静默 pass（agentHook catch）

- [x] judge 输出 schema 稳定可回归（`e6-smoke.js` 规则降级）

- [ ] 在线 E6：Agent 平台 + LLM 全链路（需部署 1.5）
...
2. Agent 平台加载三个新插件目录（或 `PLUGIN_DIR` 指向 `agent-management-sub/plugins`）  

3. `TESTGEN_BFF_URL` 指向 testgen BFF 端口（默认 5202）  

4. 离线自检：`node backend/scripts/e6-smoke.js`
