...
| 层级 | 在哪配置 | 进入请求的方式 |
|------|----------|----------------|
| **执行环境** | 项目管理 → 环境；或 Fitness「环境与服务端点」 | `base_url`/`bff_coach_url` 作为主机；`auth_configured.global_headers` 与**项目请求头**合并进请求；`fixed_params` 进入变量池 |
| **项目请求头** | 项目管理 → 请求头 | 全项目默认 Headers；合并权重：`{...环境 global_headers, ...项目请求头, ...用例 Headers}`；执行前不因缺 Authorization 阻断 |
