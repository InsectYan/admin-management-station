<template>
  <PageShell :title="`方案配置 — ${schemeType?.toUpperCase()}`" v-loading="loading">
    <el-alert type="info" :closable="false" style="margin-bottom:16px">
      配置页预填来自 test_item_detail；保存后可在执行确认页一键执行。执行引擎详见 nodes.md。
    </el-alert>
    <component :is="configComponent" v-if="item" :item="item" v-model="configJson" v-model:threshold="thresholdJson" />
...
import { fetchTestItem, saveRunConfig } from '@/services/fitnessService.js';
