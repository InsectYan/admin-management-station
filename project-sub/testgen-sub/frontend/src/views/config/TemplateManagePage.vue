    ...
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      ...
      <el-tab-pane label="模板列表" name="templates">
        <el-table v-loading="loading" :data="templateRows" border stripe size="small" max-height="calc(100vh - 220px)">
          ...
          <el-table-column label="关联大类" min-width="200">
            <template #default="{ row }">
              <el-tag
                v-for="m in (row.linked_majors || []).slice(0, 4)"
              ...
              <el-tag v-if="(row.linked_majors || []).length > 4" size="small" type="info">
                +{{ row.linked_majors.length - 4 }}
              </el-tag>
            </template>
          </el-table-column>
